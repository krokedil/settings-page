		...
		try {
			switch ( $current_subsection ) {
				...
				default:
					if ( is_string( $general_content ) ) {
						...
						throw new \InvalidArgumentException( 'General content must be a string or a callable.' );
					}
					break;
			}
			...
			if ( is_callable( $fallback_content ) ) {
				...
				try {
					...
					echo (string) ob_get_clean(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Buffered output is generated by trusted fallback renderer.
